# -*- coding: utf-8 -*-
"""
Created on Tue Aug 22 09:49:18 2017
...
advertiser = 'SmartCloud2'
if len(sys.argv) > 1:
    advertiser = sys.argv[1]
train_file='../data/1traindata500k.txt'             #training file
test_file='../data/2testdataall.txt'                   #test file
fm_model_file='../data/5modelFMall.txt'                    #fm model file
...
name_field = { 'City':0 ,'District':1,'Channel':2,'Domain':3,'Slot':4,'Tag':5,'Hour':6,'Terminal':7,'Agent':8,'Material':9}
#'weekday':0, 'hour':1, 'useragent':2, 'IP':3, 'region':4, 'city':5, 'adexchange':6, 'domain':7, 'slotid':8,
...
########################input_FM.MODEL.DATASET###############################################
log_p('data_source:'+str(advertiser))
feat_field = {}
feat_weights = {}
w_0 = 0
feat_num = 0
...
def feat_layer_one_index(feat, l):########feature--->layer1 index############
    return 1 + feat_field[feat] * k + l

def feats_to_layer_one_array(feats): ##############matrix product################
    x = numpy.zeros(xdim)
    x[0] = w_0
    for feat in feats:
...
log_p('This is the parameter setting:'+'\n'+'|dropout:'+str(dropout)+'|batch_size:'+str(batch_size)+' | input dimension of X:'+str(xdim) + ' | Hidden layer 1:'+str(hidden1)+ ' | Hidden layer 2:'+str(hidden2)+
        ' | Learning_rate:'+str(lr)+ ' | Activation function 1:'+ str(acti_type)+
        ' | lambda for :'+str(lambda1)
        )
        
# initialise parameters
print "intialize first layer weight for training the FNN"
...
print "intialize second layer weight for training the FNN"
...
print "intialize third layer weight for training the FNN"
ww3=numpy.zeros(hidden2)


print"Declare Theano symbolic variables"
x = T.matrix("x")
y = T.vector("y")
...
print"Construct Theano expression graph"
...
def print_err(file,msg=''):
    auc,rmse,logloss=get_err_bat(file)
    log_p( msg + '\t' + str(auc) + '\t' + str(rmse)+'\t'+str(logloss))    


print "get error via batch"
def get_err_bat(file,err_batch=100000):
    ...
    while True:
        line=fi.readline()
        if len(line) == 0:                         
            flag=True#data is complete over
        flag_start+=1
        if flag==False:
            xx,yy = get_xy(line)
...
for i in range(epoch):
    ...
    print'\n'
    print 'Time of training model: ' + str(mins) + 'm ' + str(secs) + 's'
    print'\n'


    start_time = time.time()
    print_err(train_file,'Training error:')# train error
    print'\n'

    train_time = time.time() - start_time
    mins = int(train_time / 60)
    secs = int(train_time % 60)
    print 'Time of training error:' + str(mins) + 'm ' + str(secs) + 's'
    print'\n'


    start_time = time.time()
    auc, rmse,logloss = get_err_bat(test_file)
    test_time = time.time() - start_time
    mins = int(test_time / 60)
    secs = int(test_time % 60)
    log_p( 'Testing error:'+str(auc)+'\t'+str(rmse)+'\t'+str(logloss))
    print'\n'
    print 'Time of testing error: ' + str(mins) + 'm ' + str(secs) + 's'
    print '\n'
